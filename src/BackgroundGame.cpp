...
//ôåð÷öéä äîâãéøä àú äúîåðú ø÷ò ùì ëì ùìá
...
//ôåð÷öéä äîàúçìú àú îñôø äùìá
void BackgroundGame::setLevel(int level)
{
	m_level = level;
}
//ôåð÷öéä äàçøàéú ìäãôéñ àú äìåç
...
void BackgroundGame::drawRoad(RenderWindow& window)
{
	...
	for (int n = m_startPos; n < m_startPos + 300; n++)
	{
		...
		Color grass;
		if (m_level == 1)  grass = (n / 3) % 2 ? Color(0, 100, 0) : Color(34, 139, 34); //éøå÷
		else grass = (n / 3) % 2 ? Color(255 ,250 ,250) : Color(240 ,255 ,240);//ìáï
